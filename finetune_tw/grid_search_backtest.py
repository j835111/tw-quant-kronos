...
def run_grid_search(
    ...
    print(f"{'='*70}")
    header = f"{'top_k':>6} {'hold':>5} {'Ann':>8} {'Sharpe':>8} {'MaxDD':>8} {'rank':>5}"
    print(header)
    print("-" * len(header))
    ...
    # ── Plot best combo ───────────────────────────────────────────────────────
    best = results[0]
    print(f"\nBest: top_k={best['top_k']} hold={best['hold_days']}d  "
    ...
    top3_keys = [f"k{r['top_k']}_h{r['hold_days']}" for r in results[:3]]
    plot_data = {
