...
| Colab T4 (16 GB) | `config_tw_daily_t4.yaml` | 關閉（T4 無原生 bf16）|
...
> **注意**：Colab T4 使用 `config_tw_daily_t4.yaml`（`amp_dtype: "none"`，T4 無原生 bf16）；本機 RTX Pro 6000 改用 `config_tw_daily_rtx6000.yaml`（bf16 啟用）。Kronos-base pretrained 權重從 `NeoQuasar/Kronos-base` 自動下載。
